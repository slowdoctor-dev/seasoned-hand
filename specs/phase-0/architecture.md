...
| AIO Sandbox | `ghcr.io/agent-infra/sandbox:1.0.0.152` (verified via agent-infra/sandbox GH releases, May 2026). Container requires `--security-opt seccomp=unconfined` per upstream README so Chromium can sandbox internally. | 6080 (noVNC), 7681 (ttyd), 8080 (sandbox API) — host port assigned dynamically by Docker, read back via inspect | Per-session container, launched via bollard, NOT in docker-compose |
